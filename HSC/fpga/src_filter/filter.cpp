...
ac_int<1, false> system_input(t_pixel din, t_pixel *cliped_window, ac_int<1, false> *last_pixel){
   ...
   static ac_int<8, false>  r = 0;
   static ac_int<8, false>  r_filter = 0;
   static ac_int<9, false>  c = 0;
   static ac_int<9, false>  c_filter = 0;
...
***************************************************************************/
t_pixel thresholding(t_pixel pixel, static ac_int<3, false> threshold) {
...
void pixel_processing(t_pixel data_in, t_pixel &data_out, t_mcu_data mcu_data[MCU_SIZE]) {
  ...
  t_pixel pixel_filtered;
  t_pixel window[9];
  static ac_int<1, false> last_pixel;
  ac_int<4, false> frame_mod_10 = frame % 10;

  if (system_input(data_in, window, &last_pixel)) {
	// filtration using median
    pixel_filtered = median(window);

	// update histogram
    if (frame_mod_10 == 0)
      mcu_data[FPGA_HISTOGRAM + pixel_filtered]++;

	// thresholding
    data_out = thresholding(pixel_filtered, threshold);

	// the last pixel of previous frame
    if (last_pixel) {
      if (frame_mod_10 == 1 && frame != 1)
