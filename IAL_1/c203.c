...
*/
    ...
    {// vsechny prvky jsou nastaveny na '*'
        q->arr[i] = '*';
    }
    // inicializace zacatku a konce fronty
    q->f_index = 0;
    q->b_index = 0;
}
...
*/
    // fronta je prazdna, pokud oba indexy ukazuji na stejne misto
...
*/
    // fronta je plna, pokud se indexy zkrizi
...
*/
    ...
    // pres parametr c vratime znak ze zacatku fronty
    *c = q->arr[q->f_index];
}
...
*/
    ...
    // odstraneni prvku z pole - posun na dalsi index
...
*/
    ...
    // zisk prvku ze zacatku fronty
    queueFront(q, c);
    // odstraneni prvku z fronty
    queueRemove(q);
}
...
*/
    if(queueFull(q))
    {// pokud je fronta plna - chyba
        queueError(QERR_UP);
        return;
    }
    // vlozime znak na prvni volne misto
    q->arr[q->b_index] = c;
    // aktualizujeme konec fronty (ukazuje na prvni volnou polozku)
